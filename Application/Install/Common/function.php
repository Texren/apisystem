...
function register_administrator($db, $prefix, $admin, $auth){
	...
	show_msg('创始人帐号注册完成！');
//=====================================
	show_msg('开始注册apimanager帐号...');

	$sql = "INSERT INTO `[PREFIX]ucenter_member` VALUES " .
			"('2', '[NAME]', '[PASS]', '[EMAIL]', '', '[TIME]', '[IP]', 0, 0, '[TIME]', '1')";
	$password = user_md5($admin['password'], $auth);
	$sql = str_replace(
			array('[PREFIX]', '[NAME]', '[PASS]', '[EMAIL]', '[TIME]', '[IP]'),
			array($prefix, 'apimanager', $password, 'apimanager@apisystem.cn', NOW_TIME, get_client_ip(1)),
			$sql);
	//执行sql
	$db->execute($sql);

	$sql = "INSERT INTO `[PREFIX]member` VALUES ".
			"('2', '[NAME]', '0', '0', '', '0', '1', '0', '[TIME]', '0', '[TIME]', '1');";
	$sql = str_replace(
			array('[PREFIX]', '[NAME]', '[TIME]'),
			array($prefix, 'apimanager', NOW_TIME),
			$sql);
	$db->execute($sql);
	show_msg('apimanager帐号注册完成！');
//=====================================
	show_msg('开始注册apiview帐号...');

	$sql = "INSERT INTO `[PREFIX]ucenter_member` VALUES " .
			"('3', '[NAME]', '[PASS]', '[EMAIL]', '', '[TIME]', '[IP]', 0, 0, '[TIME]', '1')";
	$password = user_md5($admin['password'], $auth);
	$sql = str_replace(
			array('[PREFIX]', '[NAME]', '[PASS]', '[EMAIL]', '[TIME]', '[IP]'),
			array($prefix, 'apiview', $password, 'apimanager@apisystem.cn', NOW_TIME, get_client_ip(1)),
			$sql);
	//执行sql
	$db->execute($sql);

	$sql = "INSERT INTO `[PREFIX]member` VALUES ".
			"('3', '[NAME]', '0', '0', '', '0', '1', '0', '[TIME]', '0', '[TIME]', '1');";
	$sql = str_replace(
			array('[PREFIX]', '[NAME]', '[TIME]'),
			array($prefix, 'apiview', NOW_TIME),
			$sql);
	$db->execute($sql);
	show_msg('apiview帐号注册完成！');
}
